#include "SampleAnalyzer/User/Analyzer/stop_topN2_topN1_ana.h"
using namespace MA5;
using namespace std;
...
// -----------------------------------------------------------------------------
bool stop_topN2_topN1_ana::Initialize(const MA5::Configuration& cfg, const std::map<std::string,std::string>& parameters)
{
  ...
  //bb (H) + lep + NJ>=2 + NbJ>=1
  Manager()->AddHisto("MET_", 100, 0, 1500);
...
// -----------------------------------------------------------------------------
void stop_topN2_topN1_ana::Finalize(const SampleFormat& summary, const std::vector<SampleFormat>& files)
{
...
// -----------------------------------------------------------------------------
bool stop_topN2_topN1_ana::Execute(SampleFormat& sample, const EventFormat& event)
{
  // ***************************************************************************
...
//-------------------------------------------------------------------------------------------//
    ...
    //bb (H) + lep + met + NJ>=2 + NbJ>=1

    if(Npair>0 && reco_leps.size()>=1 && reco_jets.size()>=2 && reco_bjets.size()>=3){
